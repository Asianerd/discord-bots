...
async def on_message(message):
    ...
    if not message.author.bot:
        ...
        global_exp[selected_user_index].AddExp(message.content)
        ...
        if message.content == 'glob_rank':
            ...
            for x in range(len(global_exp[:32])):
                ...
                global_rankings_text += (
                    '\n' + (str(x + 1).rjust(2, " ")) +'. LVL:' + (sorted(global_exp, key=lambda x: x.totalExp, reverse=True)[x].level) + ' EXP:' + (str(sorted(global_exp, key=lambda x: x.totalExp, reverse=True)[x].totalExp).rjust(6, apostrosphe)) + str(await client.fetch_user(global_id_sorted[x])))
            await message.channel.send(
